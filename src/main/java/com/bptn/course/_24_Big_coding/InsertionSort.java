...
//Summary 
//First the code checks if the array is null or 0
//The for loop starts at index 1 because the first element is considered sorted by default
//In the while loop the condition compares arr[j] > current to make sure element are shifted correctly for inseration 
//Last step inserts current into its proper position which is arr[j + 1] = current
